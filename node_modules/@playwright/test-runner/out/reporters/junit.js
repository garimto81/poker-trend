...
class JUnitReporter extends reporter_1.EmptyReporter {
    constructor() {
        super(...arguments);
        this.totalTests = 0;
    ...
    onEnd() {
        ...
        const root = {
            name: 'testsuites',
            attributes: {
                id: process.env[`${base_1.ENV_PREFIX}_JUNIT_SUITE_ID`] || '',
                name: process.env[`${base_1.ENV_PREFIX}_JUNIT_SUITE_NAME`] || '',
        ...
        const outputName = process.env[`${base_1.ENV_PREFIX}_JUNIT_OUTPUT_NAME`];
        if (outputName) {
    ...
    _addTestCase(test, entries) {
        ...
        if (!test.ok()) {
            entry.children.push({
                name: 'failure',
                attributes: {
                    message: `${path_1.default.basename(test.spec.location)} ${test.spec.title}`,
                    type: 'FAILURE',
                },
