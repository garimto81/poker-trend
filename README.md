...
📅 2025년 8월 6일 (화)
⏰ 분석 기간: 최근 24시간
📊 검증된 포커 영상: 28개 (필터링율: 72%)

🤖 AI 트렌드 심층 분석
────────────────
【현재 핫트렌드】
• WSOP 2025 관련 콘텐츠가 45% 점유율로 압도적
• 하이스테이크 캐시게임 영상 급상승 (+127%)

【숨겨진 패턴】  
• 블러프 성공 영상이 일반 핸드 분석보다 3배 높은 참여율
• 오후 2-4시 업로드가 조회수 최적화 시간대
• 썸네일에 칩과 카드가 함께 있으면 +23% 클릭률

【미래 예측 (AI 분석)】
🔺 Phil Ivey 관련 콘텐츠 상승 예상 (신뢰도 78%)
🔺 초보자 가이드 수요 증가 예상 (신뢰도 82%)
🔻 온라인 포커 튜토리얼 관심 하락 예상 (신뢰도 71%)

📺 TOP 5 검증된 포커 영상
────────────────
1. 🏆 WSOP 2025 메인이벤트 파이널 테이블 → 287K views
2. 🃏 Phil Ivey의 믿을 수 없는 블러프 → 156K views  
3. 🎯 하이롤러 캐시게임 미친 핸드 → 134K views
4. 📚 초보자를 위한 포지션 플레이 → 89K views
5. 🔥 트리톤 포커 베스트 모먼트 → 76K views

💡 이번 주 쇼츠 추천 아이디어
제목: "포커에서 절대 하지 말아야 할 실수 3가지"
컨셉: 초보자들이 자주 하는 치명적 실수를 30초 안에 정리
예상 성과: 50K+ 조회수 (교육 콘텐츠 트렌드 상승 중)
#포커초보 #포커팁 #블러프 #포지션 #포커전략
```
</details>
...
<details>
<summary>YouTube API 할당량 초과</summary>

**해결 방법:**
1. Google Cloud Console에서 할당량 확인
2. 필요시 할당량 증설 신청
3. 또는 분석 주기를 조정하여 API 호출 최적화

</details>

<details>
<summary>Slack 알림이 오지 않을 때</summary>

**체크 사항:**
1. Webhook URL 유효성 확인
2. Slack 앱 권한 설정 확인  
3. GitHub Actions Secrets 설정 확인
4. [Slack 설정 가이드](docs/guides/daily-slack-setup.md) 참조

</details>

<details>
<summary>포커 콘텐츠 검증 오류</summary>

**해결 방법:**
1. `python scripts/test_poker_validator.py` 실행하여 검증 모듈 테스트
2. 캐시 파일(`validation_cache.json`) 삭제 후 재실행
