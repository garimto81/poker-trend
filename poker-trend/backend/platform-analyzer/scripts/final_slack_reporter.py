...
try:
    from encoding_utils import safe_getenv, safe_json_dump, safe_print
except ImportError:
    # fallback 함수들
...
class FinalSlackReporter:
    ...
    def __init__(self, webhook_url: str = None):
        # 안전한 환경 변수 설정
        try:
            self.webhook_url = webhook_url or safe_getenv('SLACK_WEBHOOK_URL', required=False)
        except Exception as e:
            logging.warning(f"Slack Webhook URL 설정 실패: {e}")
            self.webhook_url = None
        
        # CompetitiveAnalysisReporter 안전하게 초기화
        try:
            self.analyzer = CompetitiveAnalysisReporter()
        except Exception as e:
            logging.error(f"CompetitiveAnalysisReporter 초기화 실패: {e}")
            self.analyzer = None
    ...
    def send_to_slack(self, message: Dict = None) -> bool:
        ...
        if not self.webhook_url:
            ...
            # 안전한 JSON 출력
            try:
                preview_json = json.dumps(message, indent=2, ensure_ascii=False)[:1000]
                safe_print(preview_json)
            except Exception as e:
                logger.warning(f"JSON 미리보기 출력 실패: {e}")
...
def preview_and_send():
    ...
    # 안전한 파일 저장
    if safe_json_dump(report, preview_file, ensure_ascii=False):
        safe_print(f"\nReport saved: {preview_file}")
    else:
        safe_print(f"Error saving report to: {preview_file}")
    
    # Slack 전송 여부 확인
    print("\n" + "="*80)
    webhook_url = os.getenv('SLACK_WEBHOOK_URL')
    if webhook_url:
        safe_print("Slack Webhook URL configured")
        try:
            choice = input("\nSend to Slack? (y/n): ").strip().lower()
            if choice == 'y':
                success = reporter.send_to_slack(report)
                safe_print("Sent successfully!" if success else "Failed to send")
            else:
                safe_print("Send cancelled")
        except (KeyboardInterrupt, EOFError):
            safe_print("\nOperation cancelled by user")
    else:
        safe_print("Warning: SLACK_WEBHOOK_URL not configured")
