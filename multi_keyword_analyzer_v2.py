...
class MultiKeywordPokerAnalyzerV2:
    def __init__(self):
        load_dotenv()
    ...
    def generate_ai_insights(self):
        ...
        for i, video in enumerate(self.top_videos, 1):
            summary = (
                ...
                f"참여율: {video['engagement_rate']*100:.2f}% | {video['hours_ago']}시간 전\n"
        ...
        prompt = f"""
다음은 최근 24시간 내 업로드된 포커 관련 YouTube 콘텐츠 분석 결과입니다.
...
1. 오늘 포커 콘텐츠의 주요 트렌드 (2-3문장)
2. 가장 빠르게 조회수를 얻고 있는 콘텐츠 유형 (2-3문장)
3. 내일 제작하면 좋을 포커 콘텐츠 아이디어 (2-3개)
"""
    ...
    def send_to_slack(self):
        ...
        message = {
            "blocks": [
                ...
                },
                {
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": f"*📈 전체 통계 ({self.analysis['total_videos']}개 비디오)*\n"
                ...
                message['blocks'].append({
                    ...
                    "text": {
                        ...
                        "text": f"*{i}. {linked_title}*\n"
                                f"🎬 {video['channel']} • {video['hours_ago']}시간 전 업로드\n"
        ...
        message['blocks'].append({
            ...
            "elements": [
                {
                    "type": "mrkdwn",
                    "text": f"_최근 24시간 | 다중 키워드 검색 | 중복 제거 | {datetime.now().strftime('%Y-%m-%d %H:%M')} 기준_"
                }
            ]
        })
    ...
    def display_console_summary(self):
        ...
        if self.top_videos:
            ...
            for i, video in enumerate(self.top_videos, 1):
                ...
                print(f"   업로드: {video['hours_ago']}시간 전")

def main():
    print("="*80)
    print("다중 키워드 포커 트렌드 분석 V2")
    print("="*80)
    
    analyzer = MultiKeywordPokerAnalyzerV2()
